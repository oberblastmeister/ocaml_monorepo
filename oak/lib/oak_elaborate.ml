...
(* and subst_expr (subst : Syntax.expr Var.Map.t) (e : Syntax.expr) : Syntax.expr =
  match e with
  | Expr_var var -> Map.find subst var |> Option.value ~default:e
  | Expr_core_ty _ | Expr_universe _ | Expr_bool _ | Expr_unit -> e
  | Expr_seal { e; ty } ->
      let e = subst_expr subst e in
      let ty = subst_expr subst ty in
      Expr_seal { e; ty }
  | Expr_app { e; es } ->
      let e = subst_expr subst e in
      let es = List.map es ~f:(subst_expr subst) in
      Expr_app { e; es }
  | Expr_abs { params; body; purity } ->
      let params =
        List.map params ~f:(fun param ->
            { param with ty = subst_expr subst param.ty })
      in
      let body = subst_expr subst body in
      Expr_abs { params; body; purity }
  | Expr_ty_fun { params; ty; purity } ->
      let params =
        List.map params ~f:(fun param ->
            { param with ty = subst_expr subst param.ty })
      in
      let ty = subst_expr subst ty in
      Expr_ty_fun { params; ty; purity }
  | Expr_proj { e; field } -> Expr_proj { e = subst_expr subst e; field }
  | Expr_mod decls ->
      Expr_mod
        (List.map decls ~f:(fun decl ->
             { decl with e = subst_expr subst decl.e }))
  | Expr_ty_mod decls ->
      Expr_ty_mod
        (List.map decls ~f:(fun decl ->
             { decl with ty = subst_expr subst decl.ty }))
  | Expr_let { var; rhs; body } ->
      let rhs = subst_expr subst rhs in
      let body = subst_expr subst body in
      Expr_let { var; rhs; body }
  | Expr_ty_sing { e; ty } ->
      let e = subst_expr subst e in
      let ty = subst_expr subst ty in
      Expr_ty_sing { e; ty }
  | Expr_irrelevant ty -> Expr_irrelevant (subst_expr subst ty)
  | Expr_if { e1; e2; e3 } ->
      let e1 = subst_expr subst e1 in
      let e2 = subst_expr subst e2 in
      let e3 = subst_expr subst e3 in
      Expr_if { e1; e2; e3 } *)
...
(* let lookup_var st var : Syntax.path =
  match Map.find st.State.context var with
  | None -> fail_s [%message "Variable not found" (var : Var.t)]
  | Some ty -> begin
      match ty with
...
and infer_ty st (expr : Syntax.expr) : Effects.t * Syntax.path =
  match expr with
  ...
  | Syntax.Expr_var var ->
      ...
      failwith ""
  | Syntax.Expr_seal (expr, ty) -> failwith ""
  | Syntax.Expr_abs (_, _, _)
  | Syntax.Expr_ty_fun _
  | Syntax.Expr_proj (_, _)
  | Syntax.Expr_mod _ | Syntax.Expr_ty_mod _
  | Syntax.Expr_let (_, _, _) ->
      (* considered as opaque if the rhs or body is opaque *)
      failwith ""
...
and to_path (st : State.t) (expr : Syntax.expr) : Syntax.path =
  let effects, ty = infer_ty st expr in
  if not (Transparency.equal effects.transparency Transparent) then
    fail_s [%message "Not transparent"];
  match push_sing st ty with
  | Path_sing (path, _) -> path
  | _ -> failwith "expected singleton"

(* expr must have no sealing *)
and get_transparent_ty st expr =
  match expr with
  | Syntax.Expr_app (f, args) ->
      let f_ty = get_transparent_ty st f in
      let args_with_ty =
        List.map args ~f:(fun arg -> (arg, get_transparent_ty st arg))
      in
      let res_ty = infer_app f_ty args_with_ty in
      res_ty
  | Syntax.Expr_var var -> lookup_var st var
  | Syntax.Expr_let (var, rhs, body) ->
      let subst = Subst.of_list [ (var, rhs) ] in
      let body' = Subst.apply subst body in
      get_transparent_ty st body'
  | Syntax.Expr_seal (_, _) -> failwith "not a transparent type"
  | Syntax.Expr_abs (params, body, _purity) ->
      let st', params =
        List.fold_map params ~init:st ~f:(fun st (var, ty) ->
            let ty = to_path st ty in
            let st' = State.add var ty st in
            (st', (var, ty)))
      in
      let body_ty = get_transparent_ty st' body in
      let res_ty = Syntax.Path_ty_fun (params, body_ty) in
      res_ty
  | Syntax.Expr_ty_fun (params, ret) ->
      let st', params =
        List.fold_map params ~init:st ~f:(fun st (var, ty) ->
            let ty = to_path st ty in
            let st' = State.add var ty st in
            (st', (var, ty)))
      in
      let ret_ty = to_path st' ret in
      let res_ty =
        Syntax.Path_ty_sing (Syntax.Path_ty_fun (params, ret_ty), assert false)
      in
      res_ty
  | Syntax.Expr_proj (mod_expr, field) ->
      let mod_ty =
        match zonk st (get_transparent_ty st mod_expr) with
        | Syntax.Path_ty_mod t -> t
        | _ -> failwith "proper type"
      in
      let res_ty =
        List.find_map mod_ty ~f:(fun (field', _, ty) ->
            if String.equal field' field then Some ty else None)
        |> Option.value_exn ~message:"no field"
      in
      res_ty
  | Syntax.Expr_mod decls -> failwith ""
  | Syntax.Expr_ty_mod _ | Syntax.Expr_type | Syntax.Expr_kind
  | Syntax.Expr_ty_sing (_, _)
  | Syntax.Expr_bool _ | Syntax.Expr_ty_bool | Syntax.Expr_unit
  | Syntax.Expr_ty_unit
  | Syntax.Expr_if (_, _, _) ->
      failwith ""

and eval_whnf st (path : Syntax.path) : Syntax.path =
  match path with
  | Syntax.Path_var _ | Syntax.Path_simple_ty _ | Syntax.Path_type
  | Syntax.Path_kind
  | Syntax.Path_ty_sing (_, _)
  | Syntax.Path_ty_mod _
  | Syntax.Path_ty_fun (_, _)
  | Syntax.Path_app (_, _)
  | Syntax.Path_proj (_, _) ->
      failwith ""
...
and zonk st path = eval_whnf st path |> push_sing st

and infer_app (f_ty : Syntax.path) (args_ty : (Syntax.expr * Syntax.path) list)
    : Syntax.path =
  (* let params, ret_ty =
    match f_ty with
    | Expr_ty_fun ty -> ty
  ...
  let params_with_args =
    ...
    | Ok t -> t
    | Unequal_lengths ->
        fail_s [%message "Invalid arg length" (params : Syntax.param list)]
  in *)
  failwith ""
(* let subst =

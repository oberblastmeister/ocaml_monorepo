...
and parse_decl st (group : Shrub.group) : Syntax.expr_decl =
  Fail.run ~f:(fun h -> Fail.one_of [ (fun () -> parse_let_decl st h group) ])
...
and parse_expr_group st (group : Shrub.group) : Syntax.expr =
  ...
  Fail.one_of
    [ (fun () -> parse_mod_expr st h group); (fun () -> parse_do_expr st h group) ]
...
and parse_simple_expr_rec st (items : Items.t) : Syntax.expr =
  let@ h =
    run_or_error (lazy (error (Error.create "Invalid expression" (failwith ""))))
  in
...
and parse_expr_right_cont st expr (items : Items.t) : Syntax.expr =
  let@ h = Fail.run_or_thunk ~default:(Fn.const expr) in
  Items.one_of
    items
    [ (fun () ->
...
and parse_atom_expr st (items : Items.t) : Syntax.expr =
  let@ h =
    Fail.run_or_thunk ~default:(fun () ->
      error (Error.create "invalid atom expression" (Items.get_span items)))
  in
  Items.one_of
    items
...
and parse_fun_ty st h items : Syntax.expr =
  ...
  let params_item =
    Fail.run_or_thunk
      ...
      ~default:(fun () ->
        error (Error.create "Expected parameter list after Fun/Funct" fun_tok.index))
  in
  let params =
    List.map params_item.groups ~f:(fun group ->
      let ty = parse_expr_group st group.group in
      let var = Syntax.Var.create_initial "_" (Syntax.expr_span ty).start in
      ({ var; ty; span = Syntax.expr_span ty } : Syntax.expr_param))
  in

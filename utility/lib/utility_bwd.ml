...
let nth xs i =
  if i < 0
  then invalid_arg "Bwd.nth"
  else (
    let rec go = function
      | Empty, _ -> failwith "Bwd.nth"
      | Snoc (_, x), 0 -> x
...
let filter ~f =
  let[@tail_mod_cons] rec go = function
    | Empty -> Empty
    | Snoc (xs, x) -> if f x then Snoc ((go [@tailcall]) xs, x) else (go [@tailcall]) xs
  in
  go
;;
...
let filteri ~f =
  let[@tail_mod_cons] rec go i = function
    | Empty -> Empty
    | Snoc (xs, x) ->
      if f i x then Snoc ((go [@tailcall]) (i + 1) xs, x) else (go [@tailcall]) (i + 1) xs
  in
  go 0
;;
...
let partition ~f xs =
  let rec go xs ys zs =
    match xs with
    | Empty -> append Empty ys, append Empty zs
    | Snoc (xs, x) ->
      if f x then (go [@tailcall]) xs (x :: ys) zs else (go [@tailcall]) xs ys (x :: zs)
  in
  go xs [] []
;;
...
let partition_map ~f xs =
  let rec go xs ys zs =
    ...
    | Snoc (xs, x) ->
      (match f x with
       | Stdlib.Either.Left y -> (go [@tailcall]) xs (y :: ys) zs
       | Stdlib.Either.Right z -> (go [@tailcall]) xs ys (z :: zs))
  in
  go xs [] []
;;
...
let t_of_sexp f t = of_list (List.t_of_sexp f t)

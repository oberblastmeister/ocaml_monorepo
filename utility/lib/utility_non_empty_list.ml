...
type 'a t = ( :: ) of 'a * 'a list [@@deriving equal, compare]
type 'a non_empty_list = 'a t [@@deriving equal, compare]
...
let t_of_sexp f s = of_list_exn (List.t_of_sexp f s)
let sexp_of_non_empty_list = sexp_of_t
let non_empty_list_of_sexp = t_of_sexp
...
let nth_exn t n =
  ...
  | None ->
    invalid_argf
      "Non_empty_list.nth_exn %d for non-empty list of length %d"
      n
      (length t)
      ()
  | Some x -> x
